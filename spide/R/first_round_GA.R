...
#'
first_round_GA <-
  function(max, slope, midpoint, lysis,MOI,pop_size, n_iterations, n_replicates, n_core, initpop,precent_error,write) {
    ...
    CallSim <- function(x) {
      low <- c(-2,-10,-10,-10,-10,-10,  2,   2,  1,  0, 0)
      high <-
        c(6, 0,    1.5,   20,   20,    0,   20,   15,  10, 1, .0001)
      ...
      for (j in 1:n_replicates) {
        ...
        while (ly_time <  min_ly - 3) {
          ...
          ly_time <-
            rgamma(1,shape = param_list[23],rate = param_list[25])
        }
        ...
        if (length(unique(ABC_data)) != 1) {
          ...
          #check that the fit worked and save the values if it did
          if (parameterVector_sig$isThisaFit &
            ...
            Vect_slope <-
              c(Vect_slope, parameterVector_sig$slopeParam_Estimate)
            Vect_mid <-
              c(Vect_mid, parameterVector_sig$midPoint_Estimate)
    ...
    round1 <-
      DEoptim(
        CallSim,lower = low,upper = high, control = list(
          NP = pop_size, initialpop = initpop, parallelType = 1,packages = c(library(sicegar),library(spide),library(Rcpp),library(R.utils)),parVar =
            parVar,strategy = 2, c = .15, itermax = n_iterations, storepopfrom = 1, storepopfreq =
            1 ,trace = TRUE
        )
      )
    ...
    p <-
      score_pop(
        round1,max, slope, midpoint, lysis,MOI,pop_size, n_iterations, n_replicates, min_time, max_time,n_cores,precent_error,0
      )
    return(p)
  }


#helper function to sort the ending population and get the distributions of the max, slope, mid, and lysis point from it
score_pop <-
