...
trueABC <-
  function(first_round_ABC_parameters,max, slope, midpoint, lysis,  MOI, n_saves, n_replicates, n_cores, accept_crit, precent_error,write) {
    ...
    #allowable amount of variation away from know distribution
        ...
        if (length(time) > (min_ly * 2) - (3 * 2) &&
          ...
          if (length(unique(ABC_data)) != 1) {
            ...
            if (parameterVector_sig$isThisaFit &
              ...
              list(
                parameterVector_sig$maximum_Estimate,parameterVector_sig$slopeParam_Estimate,parameterVector_sig$midPoint_Estimate,param_temp,st_time +
                  ly_time
              )
...
#this function doesn't return anything, it just tells the sim to write a bunch of files about timing about a population
store_time <-
  function(first_round_ABC_parameters,max, slope, midpoint, lysis,  MOI, n_saves, n_replicates, n_cores, precent_error,lag) {
    #register how many cores are to be used when calling replicates
    registerDoMC(n_cores)
    ...
    n_saved <- 0
    i <- 0
    #for each member of the population sample evaluate replicates and write the timing, the writing is done through the Rcpp code when the flag is set

    for (i in 1:100) {
      param_temp_var <- first_round_ABC_parameters[i,]
      ...
      Res2 <- foreach(j = 1:n_replicates) %dopar% {
        ...
        if (length(time) > (min_ly * 2) - (3 * 2) &&
          ...
          ABC_data <- avepa(param_temp,MOI,sim_min_time, sim_max_time,1)
